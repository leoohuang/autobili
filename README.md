...
| `MAX_TRANSCRIPT_CHARS` | `24000` | 送进解构 Prompt 的字幕上限字符数，最小 `1000` |

`MAX_TRANSCRIPT_CHARS` 用来兜住长视频：超过上限的字幕会在送进模型前按句子边界截断，并在文本末尾附一段说明，告诉模型这只是前段内容。如果你用的是小上下文模型（`gpt-4` 8k、`gpt-3.5-turbo` 16k），把它调小；用 `gpt-4o` 想多分析一些就调大。
...
- 自动解析常见 B 站输入形式：完整链接、带参数链接、直接输入 BV 号、`b23.tv` 短链、App 分享文案（如 `【标题-哔哩哔哩】 https://b23.tv/xxx`）、省略 `https://` 的裸链接、以及把 BV 号放在查询参数里的稍后再看/收藏夹链接（如 `.../list/watchlater?bvid=BV1xxxxxxxxx&oid=123`）
- 自动尝试多分页视频的不同 `cid`，不是只盯第一页
- 同一页有多条字幕轨时优先使用中文字幕（含 `ai-zh` 机器字幕），并跳过没有可用 `subtitle_url` 的占位轨（锁定轨 / 仍在生成的 AI 字幕）
...
- 目前只支持有官方字幕的 B 站视频。
- 超长视频（公开课、纪录片、长访谈）的字幕会被截断到 `MAX_TRANSCRIPT_CHARS`，解构只基于视频前段内容。
